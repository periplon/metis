...
use crate::types::{
    ...
    DatabaseType, DataFusionConfig, DataLake,
};
...
pub fn Resources() -> impl IntoView {
    ...
    // Available tags - updated when resources load
                    ...
                    resources.get().map(|data| {
                        match data {
                            Some(list) if !list.is_empty() => {
                                ...
                                let tags = extract_tags(&list, |r: &Resource| r.tags.as_slice());
                                    ...
                                    <div>
                                        ...
                                        {(total_filtered != total_count).then(|| view! {
                                            <p class="text-sm text-gray-500 mb-4">
                                                "Showing " {total_filtered} " of " {total_count} " resources"
                                            </p>
                                        })}
...
pub fn ResourceForm() -> impl IntoView {
    ...
    let mock_db_type = RwSignal::new(DatabaseType::Sqlite);
    let mock_datafusion_config = RwSignal::new(DataFusionConfig::default());

    // Load data lakes for DataFusion configuration
    let data_lakes_resource = LocalResource::new(move || async move {
        api::list_data_lakes().await.ok().unwrap_or_default()
    });
    let data_lakes = Signal::derive(move || {
        data_lakes_resource.get().unwrap_or_default()
    });
    ...
    view! {
        <div class="p-6 max-w-4xl mx-auto">
            <div class="flex items-center gap-4 mb-6">
                <a href="/resources" class="text-gray-500 hover:text-gray-700">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                    </svg>
                </a>
                <h2 class="text-2xl font-bold">"New Resource"</h2>
            </div>
            ...
            <form on:submit=on_submit class="bg-white rounded-lg shadow p-6">
                ...
                <ResourceFormFields
                    ...
                    mock_db_type=mock_db_type
                    mock_datafusion_config=mock_datafusion_config
                    data_lakes=Signal::derive(move || data_lakes.get())
                />
...
fn ResourceFormFields(
    ...
    mock_db_url: RwSignal<String>,
    mock_db_query: RwSignal<String>,
    mock_db_type: RwSignal<DatabaseType>,
    mock_datafusion_config: RwSignal<DataFusionConfig>,
    #[prop(into)]
    data_lakes: Signal<Vec<DataLake>>,
) -> impl IntoView {
    let (active_tab, set_active_tab) = signal(ResourceFormTab::Basic);
    ...
    view! {
        <div>
                ...
                <nav class="flex -mb-px space-x-8">
                    <button
                        ...
                        class=move || format!(
                            ...
                            if active_tab.get() == ResourceFormTab::Basic {
                                "border-blue-500 text-blue-600"
                            } else {
                                "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"
                            }
                        )
                        on:click=move |_| set_active_tab.set(ResourceFormTab::Basic)
                    >
                        <span class="flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button
                        ...
                        class=move || format!(
                            "py-2 px-1 border-b-2 font-medium text-sm transition-colors {}",
                            if active_tab.get() == ResourceFormTab::Schema {
                                "border-blue-500 text-blue-600"
                            } else {
                                "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"
                            }
                        )
                        on:click=move |_| set_active_tab.set(ResourceFormTab::Schema)
                    >
                        <span class="flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button
                        ...
                        class=move || format!(
                            "py-2 px-1 border-b-2 font-medium text-sm transition-colors {}",
                            if active_tab.get() == ResourceFormTab::Response {
                                "border-blue-500 text-blue-600"
                            } else {
                                "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"
                            }
                        )
                        on:click=move |_| set_active_tab.set(ResourceFormTab::Response)
                    >
                        <span class="flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            // Tab Content
            // Basic Info Tab
            <div style=move || if active_tab.get() == ResourceFormTab::Basic { "display: block" } else { "display: none" }>
            ...
            // Schema Tab
            <div style=move || if active_tab.get() == ResourceFormTab::Schema { "display: block" } else { "display: none" }>
            ...
            // Response Tab
            <div style=move || if active_tab.get() == ResourceFormTab::Response { "display: block" } else { "display: none" }>
                <div class="mb-4">
    ...
    view! {
        {move || {
            ...
            match strat.as_str() {
                "none" | "static" => {
                    if has_schema_properties() {
                        ...
                        view! {
                            <JsonEditor
                                value=content
                                placeholder=r#"{"data": "static content"}"#.to_string()
                                rows=8
                                label="Static Content (JSON)".to_string()
                                help_text="Enter the JSON content that will be returned for this resource".to_string()
                            />
                        }.into_any()
                    }
                }
                ...
                "database" => view! {
                    <crate::components::database_editor::DatabaseStrategyEditor
                        db_type=db_type
                        db_url=db_url
                        db_query=db_query
        ...
        wasm_bindgen_futures::spawn_local(async move {
            match api::get_resource(&decoded_uri).await {
                Ok(resource) => {
                    ...
                    content.set(resource.content.clone().unwrap_or_default());
                    ...
                    if let Some(mock) = &resource.mock {
                        ...
                        if let Some(db) = &mock.database {
                            ...
                            if let Some(df_config) = &db.datafusion {
                                mock_datafusion_config.set(df_config.clone());
                            }
                        }
                    }
    ...
    view! {
        <div class="p-6 max-w-4xl mx-auto">
            <div class="flex items-center gap-4 mb-6">
                ...
                <h2 class="text-2xl font-bold">"Edit Resource"</h2>
            </div>
