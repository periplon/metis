...
pub fn Agents() -> impl IntoView {
    ...
    let do_send_message = move || {
        ...
        if let Some(agent) = test_target.get() {
            ...
            let args = serde_json::json!({ "prompt": input });
                ...
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg border border-gray-200 dark:border-gray-700 p-6 space-y-6">
                    ...
                    {move || {
                        if agent_type.get() == AgentType::ReAct {
                            let tools = all_tools.get();
                            let regular_tools: Vec<_> = tools.iter().filter(|t| !t.name.starts_with("mcp__")).cloned().collect();
                            let mcp_tools: Vec<_> = tools.iter().filter(|t| t.name.starts_with("mcp__")).cloned().collect();
                            ...
                            view! {
                                <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                                    ...
                                    {if tools_loading.get() {
                                        ...
                                        view! {
                                            <div class="text-sm text-gray-500 dark:text-gray-400">"No tools available. Create tools in the Tools section first."</div>
                                        }.into_any()
                                    } else {
                                        view! {
                                            <div class="space-y-4">
                                                    ...
                                                    view! {
                                                        <div>
                                                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                                                "Available Tools "
                                                                <span class="text-gray-400 font-normal">"("{regular_tools.len()}" available)"</span>
                                                            </label>
                                                            <div class="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto p-2 border border-gray-200 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-900">
                                                                {regular_tools.into_iter().map(|tool| {
                                                                    let tool_name = tool.name.clone();
                                                                    let tool_name_for_check = tool_name.clone();
                                                                    let tool_name_for_toggle = tool_name.clone();
                                                                    let tool_desc = tool.description.clone();
                                                                    view! {
                                                                        <label class="flex items-start gap-2 p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded cursor-pointer">
                                                                            <input
                                                                                type="checkbox"
                                                                                class="mt-1 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                                                                                prop:checked=move || selected_tools.get().contains(&tool_name_for_check)
                                                                                on:change=move |_| {
                                                                                    let mut current = selected_tools.get();
                                                                                    if current.contains(&tool_name_for_toggle) {
                                                                                        current.retain(|t| t != &tool_name_for_toggle);
                                                                                    } else {
                                                                                        current.push(tool_name_for_toggle.clone());
                                                                                    }
                                                                                    set_selected_tools.set(current);
                                                                                }
                                                                            />
                                                                            <div class="flex-1 min-w-0">
                                                                                <div class="text-sm font-medium text-gray-900 dark:text-white truncate">{tool_name}</div>
                                                                                <div class="text-xs text-gray-500 dark:text-gray-400 truncate">{tool_desc}</div>
                                                                            </div>
                                                                        </label>
                                                                    }
                                                                }).collect::<Vec<_>>()}
                                                            ...
                                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                                                {move || format!("{} selected", selected_tools.get().len())}
                                                            </p>
                                                        </div>
                                                    }.into_any()
                                                } else {
                                                ...
                                                {if !mcp_tools.is_empty() {
                                                    view! {
                                                        <div>
                                                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                                                "MCP Tools "
                                                                <span class="text-purple-500 font-normal">"("{mcp_tools.len()}" from external servers)"</span>
                                                            </label>
                                                            <div class="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto p-2 border border-purple-200 dark:border-purple-800 rounded-md bg-purple-50 dark:bg-purple-900/20">
                                                                {mcp_tools.into_iter().map(|tool| {
                                                                    let tool_name = tool.name.clone();
                                                                    let tool_name_for_check = tool_name.clone();
                                                                    let tool_name_for_toggle = tool_name.clone();
                                                                    let tool_desc = tool.description.clone();
                                                                    // Extract server name from mcp__{server}_{tool}
                                                                    let display_name = tool_name.strip_prefix("mcp__")
                                                                        .map(|s| s.replacen('_', ":", 1))
                                                                        .unwrap_or_else(|| tool_name.clone());
                                                                    view! {
                                                                        <label class="flex items-start gap-2 p-2 hover:bg-purple-100 dark:hover:bg-purple-900/40 rounded cursor-pointer">
                                                                            <input
                                                                                type="checkbox"
                                                                                class="mt-1 rounded border-purple-300 text-purple-600 focus:ring-purple-500"
                                                                                prop:checked=move || selected_mcp_tools.get().contains(&tool_name_for_check)
                                                                                on:change=move |_| {
                                                                                    let mut current = selected_mcp_tools.get();
                                                                                    if current.contains(&tool_name_for_toggle) {
                                                                                        current.retain(|t| t != &tool_name_for_toggle);
                                                                                    } else {
                                                                                        current.push(tool_name_for_toggle.clone());
                                                                                    }
                                                                                    set_selected_mcp_tools.set(current);
                                                                                }
                                                                            />
                                                                            <div class="flex-1 min-w-0">
                                                                                <div class="text-sm font-medium text-purple-900 dark:text-purple-200 truncate">{display_name}</div>
                                                                                <div class="text-xs text-purple-600 dark:text-purple-400 truncate">{tool_desc}</div>
                                                                            </div>
                                                                        </label>
                                                                    }
                                                                }).collect::<Vec<_>>()}
                                                            ...
                                                            <p class="mt-1 text-xs text-purple-600 dark:text-purple-400">
                                                                {move || format!("{} selected", selected_mcp_tools.get().len())}
                                                            </p>
                                                        </div>
                                                    }.into_any()
                                                } else {
